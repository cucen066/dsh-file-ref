					...
					try {
						...
						return data.files
							...
							.map((f) => {
								...
								return {
									name: f.name,
									// Only show the relative path when it adds
									// information (file inside a subdirectory);
									// for root-level files it equals the name
									// and would render the filename twice.
									description: rel === f.name ? void 0 : rel,
									path: f.path,
									icon: "📄"
								};
							});
