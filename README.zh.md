...
面向 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) Web 界面的 **工作区文件引用** 插件（类 Codex 体验）：在输入框输入 `@` 即可浏览当前工作区的文件，选中后以纯文本插入该文件相对工作区的路径。

```
@ → 情景模拟对话主题_男女_男男.md → 发送
```

Agent 收到的是干净的工作区相对路径，可以直接读取；没有绝对路径，也没有被截断的引用块。
...
- 发送内容**所见即所得**，就是你选中的那个；
- Agent 收到的是简短、无歧义的相对路径（子目录文件为 `子目录/文件名`）。
